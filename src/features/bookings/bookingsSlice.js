...
export const checkInBooking = createAsyncThunk(
    'bookings/checkIn',
    async ({ bookingId, guestId, datiDocumento, roomId, parkingId }, { rejectWithValue }) => {
        try {
            await localApi.patch(`/guests/${guestId}`, datiDocumento)
            const response = await localApi.patch(`/bookings/${bookingId}`, { stato: 'in_corso' })
            await localApi.patch(`/rooms/${roomId}`, { occupata: true })
            if (parkingId) {
                await localApi.patch(`/parkings/${parkingId}`, { occupato: true })
            }
            return response.data
        } catch (error) {
            return rejectWithValue(error.message)
